...
_db_path = os.path.join(PROJECT_ROOT, "db")
if sys.platform == "win32" and not os.path.isfile(_db_path):
    _db_path += ".exe"
DB_BINARY = _db_path
TEST_DB = os.path.join(PROJECT_ROOT, "test.db")
...
class TestDatabase(unittest.TestCase):
    ...
    def run_script(self, commands):
        ...
        # Match Ruby's String#split("\n") behavior: drop trailing empty strings
        lines = stdout.split("\n")
    ...
    def test_allows_printing_out_the_structure_of_a_3_leaf_node_btree(self):
        ...
        script.append(".exit")
        result = self.run_script(script)
        self.assertCountEqual(result[14:], [
            "db > Tree:",
            "- internal (size 1)",
            "  - leaf (size 7)",
            "    - 1",
    ...
    def test_allows_printing_out_the_structure_of_a_4_leaf_node_btree(self):
        ...
        result = self.run_script(script)
        self.assertCountEqual(result[30:], [
            "db > Tree:",
            "- internal (size 3)",
            "  - leaf (size 7)",
            "    - 1",
    ...
    def test_allows_printing_out_the_structure_of_a_7_leaf_node_btree(self):
        ...
        result = self.run_script(script)
        self.assertCountEqual(result[64:], [
            "db > Tree:",
            "- internal (size 1)",
            "  - internal (size 2)",
            "    - leaf (size 7)",
    ...
    def test_prints_all_rows_in_a_multi_level_tree(self):
        script = [f"insert {i} user{i} person{i}@example.com" for i in range(1, 16)]
        script.append("select")
        script.append(".exit")
        result = self.run_script(script)
        self.assertCountEqual(result[15:], [
            "db > (1, user1, person1@example.com)",
